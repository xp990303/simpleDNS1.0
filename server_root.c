...
int main(int argc, char *argv[])
{
    ...
    FILE *fd;
    if ((fd = fopen("dns_root.txt", "r+")) == NULL)
    {
    ...
    while (1)
    {
        ...
        printf("serverroot:receivefrom:%s %d\n", inet_ntoa(cli_addr.sin_addr), cli_addr.sin_port);
		...
		printf("serverroot:name=%s,dnsqer->name=%s\n",name,dnsqer->name);
        for (i = strlen(name); name[i - 1] != '.'; i--)
        {
        }
        strcpy(name_ns, name + i);
		
        flag = FindCache(buf_file, name_ns, fd);
		printf("serverroot:name_ns=%s,flag=%d\n",name_ns,flag);
        if (flag == 1) //在缓存中
        {
		...
		printf("serverroot:send_len:send_len=%d\n",send_len);
    }
    close(sockfd);
    return 0;
}
